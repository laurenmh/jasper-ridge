...
JR_soil <- JRsoil %>%
  mutate(soilDepth = depth)

JR_gopher <- JRgopher

JR_gopher_mean <- left_join(JR_gopher, JR_soil) %>%
  group_by(soilDepth, quadID, treatment, trtrep) %>%
...
JR_tog0 <- left_join(JRcover, JR_soil)
JR_tog1 <- left_join(JR_tog0, JR_rain) %>%
  mutate(uniqueID = quadID)
...
JR_mean0 <- JR_tog %>%
  group_by(Genus, Species, LifeForm, species, soilDepth, quadID, treatment, trtrep) %>%
...
theme_update( panel.grid.major=element_blank(), panel.grid.minor=element_blank(),
              ...
              axis.text = element_text(size = 14))
# 
#  ggplot(JR_mean_time_focal, aes(x=year, y=meancover, lty = treatment, shape = treatment, color = treatment)) + geom_line() + 
...
#pdf("mastergraph-smallplots.pdf", width = 16, height = 24)
